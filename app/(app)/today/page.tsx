...
export default async function TodayPage() {
  ...
  const openChores = today.myChores.filter(
    (chore) => chore.status !== "confirmed",
  );
  ...
  return (
    <>
      ...
      {/*
        Everything in the main column is something to act on — chores to tap,
        a queue to answer, what the house has said. Everything in the rail is
        something to glance at: who is in, what today cost, what there is to
        eat. On a phone they stack, and the order is the one this screen has
        always had.
      */}
      <Columns
        main={
          <>
            {/*
              ...
              control marks it, and the photo and the note open after the
            ...
            */}
            ...
            {/*
              Needs you — chore confirmations, expense approvals and decisions in
              one list, ordered by urgency rather than by kind. A Critical
              decision carries the accent rule and sits above everything.
            */}
        ...
        aside={
          <>
            {/* Who is here, and who is not. One line, because on most days it is
                one fact and it does not deserve a card. */}
            <Section
              ...
              <ul className="flex flex-wrap gap-x-4 gap-y-1.5">
                {today.presence.home.map((member) => (
                  ...
                  >
                    <span
                      aria-hidden
                      className="h-1.5 w-1.5 rounded-full bg-text"
                    />
                    {member.displayName}
                  </li>
                ))}
                {today.presence.away.map((member) => (
                  ...
                  >
                    <span
                      aria-hidden
                      className="h-1.5 w-1.5 rounded-full border border-border-strong"
                    />
                    {member.displayName}
            ...
            {/*
              What today cost, and what is still unapproved in it. The 2.0
              version printed one run-on sentence joining two expenses with an
              em dash; a list of amounts is the thing being asked about.
            */}
            {today.money.expenses.length > 0 ? (
              ...
              >
                <div className="rounded-[var(--radius-lg)] border border-border bg-surface p-4">
                  <Readout
                    value={formatMoney(today.money.totalPaise, { currency })}
                    size="lg"
                  />
            ...
            {/*
              Food. The one block that is shown even when it holds nothing,
              because the prompt is the point: an unrecorded meal is the
              commonest gap in the home's record, and asking is what closes it.
            */}
              ...
              <div className="rounded-[var(--radius-lg)] border border-border bg-surface p-4">
                ...
                {today.food.suggestions.length > 0 ? (
                  <p className="caption-text mt-1.5 text-text-muted">
                    ...
                    {today.food.suggestions
                      .map((food) => food.name)
                      .join(" · ")}
                  </p>
                ) : today.food.coldStart ? (
                  <p className="caption-text mt-2 text-text-subtle">
                    Record a few meals and this starts suggesting from what the
                    home actually eats.
                  </p>
                ) : null}
              </div>
